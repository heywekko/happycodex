...
# 确认目标容器的 folder（容器名格式：happycodex-{folder}-{timestamp}）
ls data/ipc/
...
### 第四步：重置 Agent 会话（关键：清除 token 上下文）
...
- **必须重置 session**：仅删除 DB 消息不够，Agent session 文件中仍保存着历史上下文，下次激活时依然消耗 token
- **容器名与 folder 的对应关系**：容器名格式为 `happycodex-{folder}-{timestamp}`，可通过 `ls data/ipc/` 查看所有 folder
