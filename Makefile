.PHONY: dev dev-backend dev-web build build-backend build-web start \
       ...
       format format-check install install-host-tools clean reset-init sync-types \
...
# 单行 shell 片段：运行 dev 命令前暂停 pm2 中的 happycodex，退出（正常/中断/终止）时恢复。
# 用法示例：@$(PM2_GUARD); <command>
PM2_GUARD = PM2_WAS_RUNNING=0; \
	if command -v pm2 >/dev/null 2>&1 && pm2 show happycodex 2>/dev/null | grep -q 'online'; then \
	  PM2_WAS_RUNNING=1; \
	  echo "⏸  暂停 pm2 happycodex..."; \
	  pm2 stop happycodex; \
	fi; \
	trap "if [ \"$$PM2_WAS_RUNNING\" = '1' ]; then echo '▶  恢复 pm2 happycodex...'; pm2 start happycodex; fi" EXIT INT TERM
...
start: ## 一键启动生产环境（pm2 托管时自动走 pm2 restart；否则前台阻塞）
	@# pm2 注册过 happycodex 就路由到 pm2，避免裸跑和 pm2 抢端口
	@if command -v pm2 >/dev/null 2>&1 && pm2 describe happycodex >/dev/null 2>&1; then \
...
_start-pm2: ## (内部) pm2 托管模式：build 后 pm2 restart
	@echo "🔄 检测到 pm2 托管 happycodex，改走 pm2 restart"
	@$(MAKE) _check-sync _build-web-if-stale _build-ar-if-stale _build-backend-if-stale
	@pm2 restart happycodex --update-env
	@sleep 2
	@pm2 logs happycodex --lines 20 --nostream || true
	@echo "✅ 启动完成，查看实时日志：pm2 logs happycodex"
...
logs: ## 实时查看日志（需配合手动后台运行：make start > /tmp/happycodex.log 2>&1 &）
	@tail -f /tmp/happycodex.log

stop: ## 停止服务（pm2 托管时走 pm2 stop，否则杀端口监听进程）
	@if command -v pm2 >/dev/null 2>&1 && pm2 describe happycodex >/dev/null 2>&1; then \
	  pm2 stop happycodex >/dev/null && echo "✅ 已 pm2 stop happycodex（需再起用 pm2 start happycodex）"; \
	else \
	  lsof -ti:$(PORT) -sTCP:LISTEN 2>/dev/null | xargs kill 2>/dev/null && echo "✅ 已停止 HappyCodex (端口 $(PORT))" || echo "⚠️  端口 $(PORT) 未被占用，无需停止"; \
	fi

status: ## 查看服务运行状态
	@echo "=== HappyCodex 服务状态 ==="
	@if command -v pm2 >/dev/null 2>&1 && pm2 describe happycodex >/dev/null 2>&1; then \
	  echo "🔧 pm2 托管模式（重启请用 pm2 restart happycodex，勿混用 make start/stop）"; \
	  pm2 describe happycodex 2>/dev/null | grep -E "status|pid|uptime|restarts" | head -4 | sed 's/^/   /'; \
	fi
	@if lsof -ti:$(PORT) -sTCP:LISTEN >/dev/null 2>&1; then \
	...
	@echo ""
	@echo "=== 日志文件 ==="
	@if [ -f /tmp/happycodex.log ]; then \
	  echo "✅ /tmp/happycodex.log 存在 ($(wc -l < /tmp/happycodex.log) 行)"; \
	  echo "   最近 3 行:"; \
	  tail -3 /tmp/happycodex.log | sed 's/^/   /'; \
	else \
	  echo "⚠️  /tmp/happycodex.log 不存在（未用后台模式启动）"; \
	fi
	@echo ""
	@echo "=== Docker 容器 ==="
	@docker ps --filter "name=happycodex" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null || echo "   Docker 未运行或无 HappyCodex 容器"
...
_ensure-docker-image: ## (内部) 检测 Docker 镜像是否需要构建/重建
	@if command -v docker >/dev/null 2>&1; then \
	  if ! docker image inspect happycodex-agent:latest >/dev/null 2>&1; then \
...
backup: ## 备份运行时数据到 happycodex-backup-{date}.tar.gz
	@DATE=$$(date +%Y%m%d-%H%M%S); \
	FILE="happycodex-backup-$$DATE.tar.gz"; \
	echo "📦 正在打包备份到 $$FILE ..."; \
	tar -czf "$$FILE" \
	  --exclude='data/ipc' \
	  --exclude='data/env' \
	  --exclude='data/happycodex.log' \
...
restore: ## 从 happycodex-backup-*.tar.gz 恢复数据（用法：make restore 或 make restore FILE=xxx.tar.gz）
	@if [ -n "$(FILE)" ]; then \
	  BACKUP="$(FILE)"; \
	elif [ $$(ls happycodex-backup-*.tar.gz 2>/dev/null | wc -l) -eq 1 ]; then \
	  BACKUP=$$(ls happycodex-backup-*.tar.gz); \
	elif [ $$(ls happycodex-backup-*.tar.gz 2>/dev/null | wc -l) -gt 1 ]; then \
	  echo "❌ 发现多个备份文件，请用 make restore FILE=xxx.tar.gz 指定："; \
	  ls happycodex-backup-*.tar.gz; \
	  exit 1; \
	else \
	  echo "❌ 未找到备份文件，请将 happycodex-backup-*.tar.gz 放到当前目录"; \
	  exit 1; \
	fi; \
	echo "📂 正在从 $$BACKUP 恢复..."; \
