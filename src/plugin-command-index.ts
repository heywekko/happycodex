 ...
 *
 * Per-user index of slash commands contributed by enabled HappyCodex plugins.
 *
 * The agent runner surfaces non-DMI plugin commands. Commands
 ...
 * In runner mode we lose that expansion, so HappyCodex needs its own index to
 * resolve `/foo` and `/plugin:foo` slashes from IM / Web messages before they
 * reach the agent. The expander builds on top of this index; this module only
 * lists + resolves entries.
 *
 * Lookup strategy:
