...
async function clearMountedRuntimePath(
  hostPath: string,
  shellCommand: string,
  ...
  try {
    ...
    logger.error({ ...logContext, err }, 'Docker fallback cleanup failed');
  }
}
...
async function clearSessionRuntimeFiles(
  ...
  const inheritedRuntimeDir = getSessionRuntimeDir(folder, '.claude', agentId);
  if (fs.existsSync(inheritedRuntimeDir)) {
    let cleared = false;
    try {
      for (const entry of fs.readdirSync(inheritedRuntimeDir)) {
        if (entry === 'settings.json') continue;
        fs.rmSync(path.join(inheritedRuntimeDir, entry), {
          recursive: true,
          force: true,
        });
      }
      ...
      logger.info(
        { folder, agentId },
        'Direct inherited runtime cleanup failed, trying Docker fallback',
      );
    }

    if (!cleared) {
      await clearMountedRuntimePath(
        inheritedRuntimeDir,
        'find /target -mindepth 1 -not -name settings.json -exec rm -rf {} + 2>/dev/null; exit 0',
        { folder, agentId, runtime: 'inherited' },
      );
    }
  }

  const codexRuntimeDir = getSessionRuntimeDir(folder, '.codex', agentId);
  const codexSessionsDir = path.join(codexRuntimeDir, 'sessions');
  ...
  try {
    ...
    logger.info(
      { folder, agentId },
      'Direct Codex session cleanup failed, trying Docker fallback',
    );
    await clearMountedRuntimePath(
      codexRuntimeDir,
      'rm -rf /target/sessions 2>/dev/null; exit 0',
      { folder, agentId, runtime: 'codex' },
    );
  }
}
...
async function handleRecallCommand(chatJid: string): Promise<string> {
  ...
  logger.info(
    ...
    '/recall: built transcript, calling Codex',
  );

  // Try to summarize via Codex
  const summary = await summarizeWithCodex(transcript, targetFolder);
  if (summary) {
    logger.info(
...
/**
 * Summarize a conversation transcript using the isolated Codex runtime.
 */
async function summarizeWithCodex(
  ...
  return sdkQuery(prompt, { model, timeout: 30_000, groupFolder });
}
...
async function generateAndApplyLLMTitle(
  ...
  try {
    ...
    const raw = await sdkQuery(prompt, { timeout: 20_000, groupFolder });
    if (!raw) return;
...
interface SendMessageOptions {
  ...
  source?: string;
  /** Metadata used to preserve agent turn semantics for persisted messages. */
  messageMeta?: {
    turnId?: string;
    sessionId?: string;
...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  const existingRuntimeSessionId =
    getSession(effectiveGroup.folder) || undefined;
  ...
  const shouldInjectFreshHistory = !existingRuntimeSessionId && !isRecovery;
  if (isRecovery || shouldInjectFreshHistory) {
    const historyContext = buildRecentConversationHistoryContext(
      chatJid,
      new Set(missedMessages.map((m) => m.id)),
      {
        limit: isRecovery ? 20 : 30,
        maxMessageLength: isRecovery ? 500 : 700,
        intro: isRecovery
          ? '检测到上次有未完成消息，当前使用新会话恢复处理。以下是恢复前的最近对话记录，供你了解上下文。'
          : '检测到当前底层 Codex session 是新的。以下是 HappyCodex 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      ...
      logger.info(
        {
          ...
          isRecovery,
          freshRuntimeSession: shouldInjectFreshHistory,
        },
        isRecovery
  ...
  let output:
    ...
    | undefined;
  let activeSessionId = existingRuntimeSessionId;
  ...
  const isCodexResumeOverflow = errorForReset.includes(
    'codex_resume_context_overflow:',
  );
  const isCodexResumeMissingSession = errorForReset.includes(
    'codex_resume_missing_session:',
  );
  if (
    !sentReply &&
    (output.status === 'error' || hadError) &&
    (isCodexResumeOverflow || isCodexResumeMissingSession)
  ) {
    const detail = (lastError || output.error || '').replace(
      /.*codex_resume_(?:context_overflow|missing_session):\s*/,
      '',
    );
    const runtimeFolder = effectiveGroup.folder;
    const reason = isCodexResumeMissingSession ? 'missing rollout' : 'overflow';
    const resetLogMessage = isCodexResumeMissingSession
    ...
    logger.warn(
      { group: group.name, folder: runtimeFolder, error: detail },
      resetLogMessage,
    );

    try {
      await clearSessionRuntimeFiles(runtimeFolder);
    } catch (err) {
      logger.error(
        { folder: runtimeFolder, err },
        `Failed to clear Codex session files after resume ${reason}`,
      );
    }

    try {
      deleteSession(runtimeFolder);
      delete sessions[runtimeFolder];
    } catch (err) {
      logger.error(
        { folder: runtimeFolder, err },
        `Failed to clear session state after Codex resume ${reason}`,
      );
    }
...
async function processAgentConversation(
  ...
  // Get or use agent-specific session before building the prompt. If there is
  // no resumable Codex thread yet, inject persisted HappyCodex chat history so
  // the fresh session does not look like an empty conversation.
  ...
  if (!sessionId) {
    const historyContext = buildRecentConversationHistoryContext(
      ...
      new Set(missedMessages.map((m) => m.id)),
      {
        ...
        intro:
          '检测到当前 agent 的底层模型 session 是新的。以下是 HappyCodex 保存的最近对话记录，供你延续上下文。',
      },
    );
    if (historyContext) {
      prompt = historyContext.context + prompt;
  ...
  try {
    ...
    if (
      ...
      const reason = isCodexResumeMissingSession
        ? 'missing rollout'
        : 'overflow';
      const resetLogMessage = isCodexResumeMissingSession
        ? 'Codex resume session missing rollout in conversation agent, clearing session for history-backed retry'
        : 'Codex resume session overflow in conversation agent, clearing session for history-backed retry';
      logger.warn(
      ...
      try {
        ...
        logger.error(
          ...
          `Failed to clear agent session state after Codex resume ${reason}`,
        );
      }
      return;
    }
...
async function startMessageLoop(): Promise<void> {
  ...
  logger.info('happycodex running');
...
async function ensureDockerRunning(): Promise<void> {
  ...
  // Kill and clean up orphaned happycodex containers from previous runs
  try {
    const { stdout } = await execFileAsync(
      'docker',
      ['ps', '--filter', 'name=happycodex-', '--format', '{{.Names}}'],
      { timeout: 10000 },
    );
...
async function main(): Promise<void> {
  ...
  // trigger via POST /api/plugins/catalog/scan. scanHostMarketplaces reads the
  // effective external dir and has an in-flight Promise mutex, so UI
  // button / startup / periodic timer can overlap safely.
...
main().catch((err) => {
  logger.error({ err }, 'Failed to start happycodex');
  process.exit(1);
});
