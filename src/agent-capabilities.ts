...
import os from 'os';
import { resolveCodexExecutable } from './codex-runtime.js';
...
export const AGENT_CAPABILITIES: AgentCapability[] = [
  {
    ...
    binary: 'codex',
    required: true,
    installHint: 'Install Codex CLI, for example: npm install -g @openai/codex',
  },
  {
    name: 'feishu-cli',
    binary: 'feishu-cli',
 ...
 *
 * Fallback uses `which` because package-manager shims may sit in different
 * locations depending on how the host installed the CLI.
 */
async function resolveBinaryPath(binary: string): Promise<string | null> {
  if (binary === 'codex') {
    const resolved = resolveCodexExecutable(binary);
