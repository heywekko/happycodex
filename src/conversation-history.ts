...
/**
 * Build a `<system_context>` block of recent persisted HappyCodex chat history
 * to prepend to a prompt when the underlying runtime session is fresh
 * (recovery after a crash, or after clearing a stale runtime session).
 * Without this the new model sees
 ...
 * and the container/host runner. Keeping a single implementation ensures the
 * injected framing — and
 * the lone-surrogate / closing-tag sanitisation — stays byte-for-byte
 * consistent across every path that feeds the runtime.
 */
export function buildRecentConversationHistoryContext(
  ...
  const historyLines = historyMsgs.map((m) => {
    ...
    // such as emoji before injecting recent history into a runtime prompt.
  ...
  return {
    ...
    context:
      '<system_context>\n' +
      opts.intro +
      '\n重要：这些只是 HappyCodex 持久化的历史聊天记录，用来在新模型/新 session 中恢复上下文。回答当前用户消息时，请优先依据当前消息和当前文件状态；如果历史与当前问题无关，请直接忽略。\n\n' +
      historyLines.join('\n') +
      '\n</system_context>\n\n',
  };
}
