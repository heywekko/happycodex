...
function parseTomlInlineStringTable(
  ...
  for (const pair of inner.split(',')) {
    const match = pair.trim().match(/^(.+?)\s*=\s*(.+)$/);
    if (!match) continue;
...
function parseHostCodexMcpConfig(
  raw: string,
): Record<string, Partial<McpServerEntry>> {
  const servers: Record<string, Partial<McpServerEntry>> = {};
  let current: { id: string; env: boolean } | null = null;

  for (const line of raw.split(/\r?\n/)) {
    const trimmed = line.trim();
    if (!trimmed || trimmed.startsWith('#')) continue;

    const envSection = trimmed.match(
      /^\[mcp_servers\.((?:"(?:\\.|[^"])*")|[\w-]+)\.env\]$/,
    );
    if (envSection) {
      current = { id: parseTomlKey(envSection[1]), env: true };
      servers[current.id] ??= {};
      servers[current.id].env ??= {};
      continue;
    }

    const serverSection = trimmed.match(
      /^\[mcp_servers\.((?:"(?:\\.|[^"])*")|[\w-]+)\]$/,
    );
    if (serverSection) {
      current = { id: parseTomlKey(serverSection[1]), env: false };
      servers[current.id] ??= {};
      continue;
    }
    ...
    if (key === 'command') {
      const parsed = parseTomlString(value);
      if (parsed !== undefined) server.command = parsed;
    } else if (key === 'args') {
      const parsed = parseTomlStringArray(value);
      if (parsed) server.args = parsed;
    } else if (key === 'url') {
      const parsed = parseTomlString(value);
      if (parsed !== undefined) {
        server.type = 'http';
...
async function readHostCodexMcpServers(): Promise<Record<string, Partial<McpServerEntry>>> {
  const configPath = path.join(os.homedir(), '.codex', 'config.toml');
  try {
    const raw = await fs.readFile(configPath, 'utf-8');
    return parseHostCodexMcpConfig(raw);
  } catch {
    return {};
  }
}
...
// POST /sync-host — sync from host Codex MCP config (admin only)
mcpServersRoutes.post('/sync-host', authMiddleware, async (c) => {
  ...
  const hostServers = await readHostCodexMcpServers();
  ...
  if (Object.keys(hostServers).length === 0) {
    return c.json({
      ...
      deleted: 0,
      skipped: 0,
      message: 'No MCP servers found in host Codex config',
    });
  }
