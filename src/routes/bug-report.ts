...
import { getCodexRuntimeStatus } from '../codex-runtime.js';
...
async function checkCapabilities(): Promise<{
  ...
  return checkCapabilitiesForRuntime('main');
}

async function checkCapabilitiesForRuntime(groupFolder: string): Promise<{
  ghAvailable: boolean;
  ghUsername: string | null;
  codexAvailable: boolean;
}> {
  const cached = capCache.get(groupFolder);
  if (cached && Date.now() - cached.checkedAt < CAP_CACHE_TTL) {
    return {
      ghAvailable: cached.ghAvailable,
      ghUsername: cached.ghUsername,
      codexAvailable: cached.codexAvailable,
    };
  }
  ...
  const codexStatus = await getCodexRuntimeStatus(groupFolder);
  const codex = codexStatus.configured;
  ...
  capCache.set(groupFolder, { ghAvailable: gh, ghUsername, codexAvailable: codex, checkedAt: Date.now() });
  return { ghAvailable: gh, ghUsername, codexAvailable: codex };
}
...
/** Try multiple strategies to extract JSON { title, body } from Codex output */
...
bugReportRoutes.get('/capabilities', authMiddleware, async (c) => {
  ...
  const caps = await checkCapabilitiesForRuntime(folder);
  return c.json(caps);
});

/**
 * POST /api/bug-report/generate
 * Analyze the bug with Codex and generate a structured report
 */
bugReportRoutes.post('/generate', authMiddleware, async (c) => {
  ...
  // Try Codex analysis
  const caps = await checkCapabilitiesForRuntime(folder);
  if (!caps.codexAvailable) {
    logger.info('bug-report: Codex runtime not configured, using fallback template');
  ...
  try {
    logger.info(
      ...
      'bug-report: invoking Codex',
    );

    const model = process.env.RECALL_MODEL || undefined;
    const result = await sdkQuery(prompt, { model, timeout: 60_000, groupFolder: folder });
    ...
    // Try to parse Codex's JSON output
    ...
    // Codex didn't return valid JSON, use raw output as body
    logger.info('bug-report: Codex output was not valid JSON, using as raw body');
    return c.json({
      title: `Bug: ${description.slice(0, 70)}`,
      body: result,
      systemInfo,
...
bugReportRoutes.post('/submit', authMiddleware, async (c) => {
  ...
  const fullBody = `${body}\n\n---\n> Submitted via HappyCodex by ${user.display_name || user.username}`;
  ...
  const url = `https://github.com/wekko/happycodex/issues/new?title=${encodeURIComponent(title)}&body=${encodeURIComponent(truncatedBody)}`;
