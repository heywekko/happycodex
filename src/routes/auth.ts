...
authRoutes.post('/setup', async (c) => {
  ...
  return new Response(
    JSON.stringify({
      ...
      setupStatus: await buildSetupStatus(),
    }),
    {
      status: 201,
      headers: {
...
authRoutes.post('/login', async (c) => {
  ...
  const setupStatus =
    updatedUser.role === 'admin' ? await buildSetupStatus() : undefined;
