...
function resolveUserHomeFolder(userId: string): string {
  return getUserHomeGroup(userId)?.folder ?? 'main';
}
...
tasksRoutes.post('/ai', authMiddleware, async (c) => {
  ...
  void (async () => {
    try {
      ...
      const result = await sdkQuery(parsePrompt, { model, timeout: 60_000, groupFolder });
...
tasksRoutes.post('/parse', authMiddleware, async (c) => {
  ...
  try {
    const model = process.env.RECALL_MODEL || undefined;
    let groupFolder = resolveUserHomeFolder(authUser.id);
    const requestedChatJid =
      typeof body.chat_jid === 'string' && body.chat_jid ? body.chat_jid : null;
    const requestedGroupFolder =
      typeof body.group_folder === 'string' && body.group_folder ? body.group_folder : null;
    if (requestedChatJid) {
      const group = getRegisteredGroup(requestedChatJid);
      if (group && canAccessGroup({ id: authUser.id, role: authUser.role }, group)) {
        groupFolder = group.folder;
      }
    } else if (requestedGroupFolder) {
      const entry = Object.entries(getAllRegisteredGroups()).find(
        ([, candidate]) => candidate.folder === requestedGroupFolder,
      );
      const group = entry ? { ...entry[1], jid: entry[0] } : null;
      if (group && canAccessGroup({ id: authUser.id, role: authUser.role }, group)) {
        groupFolder = group.folder;
      }
    }
