 ...
 *
 * Inline-bash semantics (matches the inherited plugin CLI format):
 ...
 * how markdown renderers and agent prompts treat fences.
 */
 ...
 *     examples that should preserve literal placeholders.
...
function applyPlaceholders(
  ...
  // (matches inherited plugin behavior; bare $VAR is too greedy and would also
...
function renderExpandedPrompt(r: RenderInput): string {
  const parts: string[] = [];
  parts.push('The user manually invoked a HappyCodex plugin command:');
  parts.push('');
  ...
  parts.push('');
  parts.push(
    'Use the following command definition exactly as a user-invoked HappyCodex slash command.',
  );
  parts.push('');
  parts.push(r.body);
  return parts.join('\n');
