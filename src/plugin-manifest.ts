 ...
 * The inherited plugin CLI format lays out a placeholder dir (LICENSE/README)
 * for every declared plugin regardless of source kind, and only writes
 * `.claude-plugin/plugin.json` after install. So "declared but not yet
 * installed" is a normal pre-install state for both inline and remote — the
 * importer's actual signal is presence/absence of a marketplace.json
 * declaration, not the source kind.
 */
...
export function readMarketplaceManifest(
  ...
  // user installs them through the inherited plugin CLI. We classify each
  // entry up front so the importer can skip "remote without manifest"
  // silently while still warning on "inline without manifest" (a real
  // authoring bug).
 ...
 * — never throws. Counts are intentionally shallow (one level deep) to match
 * the inherited plugin loading contract.
 */
