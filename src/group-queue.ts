...
export class GroupQueue {
  ...
  sendMessage(
    ...
    // the task finishes. This preserves the scheduled-task IPC deferral invariant.
    //
  ...
  /**
   * Signal a specific group's active runner to gracefully exit.
   * After the runner exits, processGroupMessages will automatically restart it.
  ...
  requestGracefulRestart(groupJid: string): boolean {
    ...
    if (written) {
      ...
      logger.info(
        ...
        'Sent drain signal for graceful runner restart',
      );
    }
    return written;
  }
