...
describe('prompts/ files', () => {
  ...
  test('no prompt file contains lone UTF-16 surrogates', () => {
  ...
  test('platform prompt patches do not duplicate user rules or skill bodies', () => {
    ...
    expect(homeMemory).toContain('通过运行时挂载目录读写');
    expect(guestMemory).toContain('通过运行时挂载目录读取');
  });

  test('codex runner prompts do not require removed HappyClaw MCP tools', () => {
    const allPromptText = listMarkdownFiles(PROMPTS_DIR)
      .map((file) => fs.readFileSync(file, 'utf-8'))
      .join('\n');

    expect(allPromptText).not.toMatch(
      /memory_(?:search|get|append)|mcp__happyclaw__|`send_(?:message|image|file)`|`schedule_task`|`Read` 工具|`Edit` 工具|Task 工具/,
    );
  });
});
