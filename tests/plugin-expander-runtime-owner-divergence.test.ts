...
describe('handleWebUserMessage — #21 round-13 P2-3 must resolve effective group', () => {
  test('IM sibling without home semantics → raw group has no created_by → null', () => {
    ...
    const homeRow: RegisteredGroupShape = {
      ...
      is_home: true,
      executionMode: 'host',
      customCwd: '/workspace/admin/myrepo',
      created_by: 'admin-1',
    };
    ...
    expect(ctx!.cwd).toBe('/workspace/admin/myrepo');
  });
  ...
  test('sibling with own customCwd preserved when home has none', () => {
    const sibling: RegisteredGroupShape = {
      ...
      is_home: false,
      executionMode: null,
      customCwd: '/workspace/admin/sibling-cwd',
      created_by: null,
    };
    const homeRow: RegisteredGroupShape = {
      folder: 'main',
    ...
    expect(effectiveGroup.customCwd).toBe('/workspace/admin/sibling-cwd');
