  ...
  const rows = [
    {
      ...
      value: audit.instructions.status,
      detail: audit.instructions.runtimePath || audit.instructions.sourcePath,
    },
    {
      label: 'rules/',
      value: `${audit.rules.fileCount} 个文件${rulesLoaded ? ` · 运行时命中 ${rulesLoaded}` : ''}`,
    ...
    {
      label: 'HappyCodex prompt',
      value: `${audit.runtimePrompt.files.length} 个补丁 · ${audit.runtimePrompt.totalBytes.toLocaleString()} bytes`,
      detail: audit.runtimePrompt.files.map((file) => file.name).join(', '),
    },
  ];
