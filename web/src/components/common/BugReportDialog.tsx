...
export function BugReportDialog({ open, onClose }: BugReportDialogProps) {
  ...
  useEffect(() => {
    if (open) {
      api.get<Capabilities>('/api/bug-report/capabilities').then(setCaps).catch(() => {
        setCaps({ ghAvailable: false, ghUsername: null, codexAvailable: false });
      });
    }
  }, [open]);
  ...
  // --- Generate report (Codex analysis) ---
      ...
      <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
        ...
        {step === 1 && showConfirm && (
          <div className="text-center py-4 space-y-3">
            ...
            <p className="text-sm text-muted-foreground">wekko/happycodex</p>
          </div>
        )}
